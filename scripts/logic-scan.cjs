...
const SKIP_FILES = ["logic-health.report.json", "routeTree.gen.ts", "types.ts"];
const SKIP_DIRS = ["components/ui", "__tests__", "integrations/supabase"];

/** Files where a swallowed failure costs money, stock or access. */
const MONEY =
  /(checkout|payment|tender|discount|refund|void|drawer|shift-close|shift_close|stock|inventory|sale|bill-number|coupon|voucher|purchas)/i;
...
function walk(dir, out = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (entry.name === "__tests__" || SKIP_DIRS.some((d) => rel === d || rel.startsWith(`${d}/`)))
        continue;
      walk(full, out);
    } else if (
  ...
  const visit = (node) => {
    if (ts.isFunctionLike(node) && node.body && ts.isBlock(node.body)) {
      const name =
        ...
        (ts.isVariableDeclaration(node.parent) ? node.parent.name.getText(source) : "");
      if (name) {
        let awaited = false,
          handled = false,
          throws = false;
        ...
        const protectedCall = (call) => {
          for (let parent = call.parent; parent; parent = parent.parent) {
            if (
              ...
              parent.catchClause &&
              call.pos >= parent.tryBlock.pos &&
              call.end <= parent.tryBlock.end
            )
              return true;
            if (
        ...
        found.push({
          ...
          awaited,
          handled: handled || (calls.length > 0 && calls.every(protectedCall)),
          throws,
        });
      }
    }
...
function scanFile(file) {
  ...
  lines.forEach((raw, i) => {
    ...
    if (
      /\b(not implemented|unimplemented|coming soon|stub(bed)? out|mock(ed)? (data|response|endpoint))\b/i.test(
        t,
      ) &&
      !/placeholder=/.test(t) &&
      !isComment &&
    ...
    if (/throw new Error\(\s*["'`](TODO|not implemented)/i.test(t)) {
      add(
        ...
        line,
        t.slice(0, 140),
        "A call here throws instead of working.",
      );
    }

    // 2. Dead UI connections
    const dead = t.match(
      /\b(onClick|onSubmit|onChange|onValueChange|onCheckedChange)=\{\s*\(\s*\)\s*=>\s*(\{\s*\}|undefined|null|void 0)\s*\}/,
    );
    if (dead) {
      add(
        "warning",
    ...
    if (logOnly) {
      add(
        ...
        "Button only logs",
        line,
        `${logOnly[1]} writes to the console instead of acting.`,
    ...
    if (
      /<form(\s|>)/.test(t) &&
      !/onSubmit/.test(raw) &&
      !/onSubmit/.test(lines.slice(i, i + 6).join(" "))
    ) {
      add(
        "warning",
        "Form has no submit handler",
        line,
        "A <form> is rendered without onSubmit.",
    ...
    // 3. Input validation
    if (
      /\bNumber\(\s*e\.target\.value\s*\)/.test(t) &&
      !/\|\|\s*\d|isNaN|Number\.isFinite|clamp/.test(raw)
    ) {
      add(
        money ? "warning" : "info",
        "Unvalidated number input",
        line,
        t.slice(0, 140),
  ...
  // `parseInt` accepts bases other than 10, and `Number.parseInt` is valid too.
  // Inspect argument count instead of using a regex that mistakes radix 16 for
  // a missing radix and also matches the `parseInt` substring in property calls.
  const source = ts.createSourceFile(
    file,
    text,
    ts.ScriptTarget.Latest,
  ...
  const inspectParseInt = (node) => {
    if (ts.isCallExpression(node)) {
      const direct = ts.isIdentifier(node.expression) && node.expression.text === "parseInt";
      const numbered =
        ts.isPropertyAccessExpression(node.expression) &&
        ts.isIdentifier(node.expression.expression) &&
        node.expression.expression.text === "Number" &&
        node.expression.name.text === "parseInt";
      if ((direct || numbered) && node.arguments.length < 2) {
        const line = source.getLineAndCharacterOfPosition(node.getStart(source)).line + 1;
        add(
          "info",
          "parseInt without a radix",
          line,
          lines[line - 1].trim().slice(0, 140),
          "Pass an explicit radix.",
        );
      }
    }
    ts.forEachChild(node, inspectParseInt);
  };
  inspectParseInt(source);

  // A function that awaits and rejects is normal promise composition. Whether
  // the failure is handled can only be decided at the call site, often in a
  // different module. Reporting every such function created hundreds of
  // non-actionable findings and encouraged catches that would swallow errors.
