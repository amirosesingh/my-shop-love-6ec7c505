class SyncCoordinator {
  constructor({ pushWorker, pullWorker, publish = () => {} }) { this.pushWorker=pushWorker; this.pullWorker=pullWorker; this.publish=publish; this.running=false; this.paused=false; this.status={ phase:"idle", pending:0, failed:0, conflicts:0, lastPushAt:null, lastPullAt:null }; }
  ...
  async runNow(options) {
    ...
    try {
      ...
      this.status.phase="idle"; return { ok:true,...this.snapshot(),...pushed,...pulled };
    } catch(error) { this.status.phase="idle"; this.status.failed+=1; return { ok:false,...this.snapshot(),code:error?.code??"ESYNC",error:String(error?.message??error) }; }
