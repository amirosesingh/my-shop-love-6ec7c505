...
BEGIN
  ...
  END IF;
  IF to_regprocedure('public.settings_effective(text,text)') IS NOT NULL THEN
  ...
  END IF;
  IF to_regprocedure('public.settings_cluster_of(text)') IS NOT NULL THEN
  ...
  END IF;
  IF to_regprocedure('public.settings_upsert(text,text,jsonb)') IS NOT NULL THEN
  ...
  END IF;
  IF to_regprocedure('public.settings_sync_batch(text,text,text[])') IS NOT NULL THEN
