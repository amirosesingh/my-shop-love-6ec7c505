-- ============================================================
-- 28_app_users_id_link.sql — free public.app_users.id from any legacy
-- foreign key to the authentication account table.
-- ============================================================
-- Safe to run repeatedly, on fresh and on existing databases.
--
-- Older installs created public.app_users.id as a reference to the sign-in
-- account id. Creating a staff profile before (or without) that exact id then
-- fails with:
--    23503 insert or update on table "app_users" violates foreign key
--          constraint "app_users_id_fkey"
--
-- The link to the sign-in account lives in app_users.auth_user_id, which is
-- what the application reads, so the constraint on id is dropped and id stays
-- a plain generated primary key.
-- ============================================================
...
-- Prefer the sign-in account id for a brand new row, so databases that still
-- carry the old link keep matching ids, and everything else gets a fresh one.
...
-- Verification: no foreign key should remain on app_users.
