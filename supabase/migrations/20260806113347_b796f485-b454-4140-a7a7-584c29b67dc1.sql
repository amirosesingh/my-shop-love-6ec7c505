-- ============================================================
-- 15 · security hardening + performance indexes
-- ============================================================
...
-- for their own branch (or rows with no branch stamped on them).
...
-- sale_items follow their parent sale
...
-- booking_payments follow their booking
...
-- ---------- lock the search path on every privileged routine ----------
DO $do$
DECLARE
  f record;
  cfg text;
...
BEGIN
  ...
  LOOP
    ...
    want := cfg || ', pg_temp';
    EXECUTE format('ALTER FUNCTION %s SET search_path = %s', f.sig, want);
  END LOOP;
END $do$;

-- ---------- guard-column and foreign-key indexes ----------
