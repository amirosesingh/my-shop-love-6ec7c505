-- Persist the per-tender client transaction key used by the unique index.
-- This keeps cash, card and split-payment retries idempotent by business key
-- as well as by the deterministic payment UUID.
