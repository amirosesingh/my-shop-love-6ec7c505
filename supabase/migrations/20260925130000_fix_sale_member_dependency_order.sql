-- Keep the complete online checkout atomic while satisfying the sales.member_id
-- and payment_transactions.member_id foreign keys for newly attached members.
...
BEGIN
  ...
  -- Parent first: both the sale and its tender rows reference this member.
  ...
  IF existing_id IS NULL THEN
    INSERT INTO public.sales (
      ...
      NULLIF(s->>'authorized_at','')::timestamptz,
...
REVOKE ALL ON FUNCTION public.pos_sale_commit(jsonb,jsonb,jsonb,jsonb,jsonb,text)
  FROM PUBLIC, anon;
