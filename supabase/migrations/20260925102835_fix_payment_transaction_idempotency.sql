-- Keep tender retries idempotent across web checkout and Electron sync.
-- Additive and safe to run more than once on an existing Supabase project.
BEGIN;
...
-- Earlier deployments created this name as a non-unique lookup index. The
-- transaction makes the replacement atomic, and the pre-deployment duplicate
-- check protects the unique build from rejecting existing data.
