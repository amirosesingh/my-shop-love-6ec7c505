...
CREATE INDEX security_findings_source_idx ON public.security_findings (source, status);
CREATE INDEX security_findings_seen_idx ON public.security_findings (last_seen_at DESC);

CREATE TRIGGER security_findings_touch
  BEFORE UPDATE ON public.security_findings
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

-- Ingest routine: append-only, never reads back privileged data.
CREATE OR REPLACE FUNCTION public.security_report_findings(
  _source text,
  _deployment_ref text,
  _findings jsonb
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Nightly drift audit of the database's own security posture.
...
SELECT cron.schedule('security-selfcheck', '0 3 * * *',
