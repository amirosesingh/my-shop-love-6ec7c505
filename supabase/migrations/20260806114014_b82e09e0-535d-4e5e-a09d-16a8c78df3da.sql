...
BEGIN
  IF to_regclass('public.settings_scoped') IS NOT NULL THEN
    CREATE INDEX IF NOT EXISTS settings_scoped_key_idx ON public.settings_scoped (scope, scope_id, key);
    CREATE INDEX IF NOT EXISTS settings_scoped_override_idx ON public.settings_scoped (scope, scope_id) WHERE is_overridden;
  END IF;
END $do$;